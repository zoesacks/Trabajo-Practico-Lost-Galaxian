...
public class Juego extends InterfaceJuego {
	...
	Random gen;
	Color colorTexto;
	Image fondo1, fondoInicio1, fondoPerdio, reintentarGris, reintentarColor, abandonarGris, nivel, abandonarColor,
			volverJuegarColor, volverJugarGris, corazon, win;
	...
	public void InicioJuego() {
		...
		// Si esta reintentantando/volviendo a jugar se reinician todos los enemigos,
		// destructores y vidas como la primera vez
		if (reintento) {
			proyectilNave = null;
		...
		// moverse
		if (entorno.estaPresionada(entorno.TECLA_IZQUIERDA) || entorno.estaPresionada(TECLA_A)) {
			nave.moverIzquierda();
		}
		if (entorno.estaPresionada(entorno.TECLA_DERECHA) || entorno.estaPresionada(TECLA_D)) {
			nave.moverDerecha();
		}
		...
		for (int i = 0; i < asteroides.length; i++) {
			for (int j = 0; j < destructores.length; j++) {
				...
				if (destructores[j] != null && asteroides[i] != null
					...
					// pero que se mueva un poco y el destructor vaya a la derecha(0.78...)
					if (destructores[j].getX() <= asteroides[i].getX()) {
						...
						// si esta yendo a la derecha lo cambiamos para que vaya a la izquierda
						else {
		...
		for (int i = 0; i < this.asteroides.length; i++) {
			...
			if (this.asteroides[i] != null && proyectilNave != null
					&& proyectilNave.colisionoAsteroide(this.asteroides[i])) {
				proyectilNave = null;
			}
		...
		for (int i = 0; i < destructores.length; i++) {
			...
			if (this.destructores[i] != null && proyectilNave != null
					&& proyectilNave.colisionoDestructor(destructores[i])) {
				proyectilNave = null;
				destructores[i] = null;
				muertos++;
			}
		...
		for (int i = 0; i < proyectilesDestructor.length; i++) {
			...
			// genera los proyectiles si esta el destructor correspondiente y cada un tiempo
			// random
	...
	public void ganoPerdio() {
		...
		// si la bandera reintentar esta activada con las flechas del teclado se puede
		// cambiar a abamndonmar o presionar enter para volver al juego
		if (reintentar) {
			if (vidaDestJefe == 0) {
		...
		// si la bandera abandonar esta activada con las flechas del teclado se puede
		// cambiar a reintentar o presionar enter para salir
		if (abandonar) {
			if (vidaDestJefe == 0) {
